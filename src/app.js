...
let j = 0;
var txt = "new hive"; /* The text */
const end_pos = txt.length;
...
function typeWriter() {
  randChar = randomChars[Math.floor(Math.random() * randomChars.length)];
  written = logo.innerHTML;

  if (start_pos < end_pos && j < 3) {
    logo.innerHTML = randChar;
  }
  start_pos++;
  j++;
